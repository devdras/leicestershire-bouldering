...
export default function Areas({ areas }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Areas', href: '/areas' }]}>
            <Head title="Areas" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <div className="grid auto-rows-min gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {areas.map((area) => {
                        const tickedCount = getTickedCount(area.climb_ids);
                        const gradeData = getGradeDistribution(area.grades);
